...
import com.example.lonelywolf.stageEndActivities.SixthStageEndActivity
import kotlinx.android.synthetic.main.activity_sixth_stage.*
import java.util.*
 ...
 * 版权所有，侵权必究
 */
class SixthStageActivity : MyActivity() {
    //这些boolean变量用于控制点击事件
            ...
            when (msg.what) {
                ...
                judgeHeroDeath -> {
                    if (heroVector.dead) {
                        ...
                        val intent = Intent(this@SixthStageActivity, DefeatActivity::class.java)
                        startActivity(intent)
                        ActivityCollector.finishOneActivity(SixthStageActivity::class.java.name)
                    }
                }
                deleteWhirlWind -> {
                ...
                victoryInformation -> {
                    ActivityCollector.finishOneActivity(SixthStageActivity::class.java.name)
                    val intent = Intent(this@SixthStageActivity, SixthStageEndActivity::class.java)
                    startActivity(intent)
                }
                jumpEnd -> {
                    enemyVector0.pounce(
                        heroVector.grid, this@SixthStageActivity, layout, gridView1,
                ...
                removeIce -> {
                    layout.removeView(findViewById(R.id.id_ice))
                }
            }
        }
    }
        ...
        button5.setOnTouchListener { view, motionEvent ->
            when (motionEvent.action) {
                MotionEvent.ACTION_DOWN -> {
                    generateSkillField(
                        "天威：死灵\n天威之魂化作死灵形态\n对一名3格内的敌人" +
                                "造成${heroVector.attack + 18}(攻击力+18)点伤害并回复" +
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                if (enemyVector0.grid.contains(event.x, event.y)) {
                    generateSkillField(
                        "雪龙-世界的主宰\n攻击：${enemyVector0.attack} 护甲：${enemyVector0.defence}\n" +
                        ...
                        if (!enemyVector.dead) {
                            if (enemyVector.grid.contains(event.x, event.y)) {
                                heroVector.kingNecromancy(
                                    this, layout, enemyVector,
                                    (heroVector.attack + 18),
    ...
    private fun generateEnemy() {
        enemyVector0 = MyImageViewFactory.generateEnemy(
            R.drawable.realsnowdragon, CharacterParams(2700, 60, 60), Grid(gridView1, 7, 3),
            layout, this, 0
        )
    ...
    private fun enemyAct(index: Int, turn: Int) {
        ...
        if (!enemyVector.dead) {
            if (index == 0) {
                enemyVector.attack += 5
                if (enemyVector.grid.radiusContain(1, heroVector.grid)) {
                    enemyVector.bite(heroVector, 3 * heroVector.attack, false, this, layout)
                }
                if (heroVector.frozen) {
                    enemyVector.dragonMagic(heroVector, this, layout, gridView1)
                    thread {
                        val message = Message()
                        message.what = removeIce
                        Thread.sleep(2000)
                        handler.sendMessage(message)
                    }
                    heroVector.frozen = false
                } else {
                    enemyVector.snowStorm(
                        heroVector,
                        this,
                        layout,
                        kingShieldOn,
